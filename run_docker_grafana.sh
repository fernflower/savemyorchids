...
influx_port=$(get_val "INFLUX_PORT")
influx_user=$(get_val "INFLUX_USER")
influx_pass=$(get_val "INFLUX_PASSWORD")
influx_dbname=$(get_val "INFLUX_DBNAME")
influx_host=$(get_val "INFLUX_HOST")
...
curl "http://$grafana_user:$grafana_pass@$grafana_host:3000/api/datasources" -X POST -H 'Content-Type: application/json;charset=UTF-8' --data-binary "{\"name\":\"$GRAFANA_SOURCE_NAME\",\"type\":\"influxdb\",\"url\":\"http://$influx_host:$influx_port\",\"access\":\"proxy\",\"isDefault\":true,\"database\":\"$influx_dbname\",\"user\":\"$influx_user\",\"password\":\"$influx_pass\"}"
