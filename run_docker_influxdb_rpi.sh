...
sudo docker run -d -p 8083:8083 -p "$influx_port:$influx_port" -e ADMIN_USER="$influx_user" -e INFLUXDB_INIT_PWD="$influx_pass" -e PRE_CREATE_DB="$influx_dbname" --volume /var/influxdb/:/data --expose 8090 --expose 8099 --name influxdb $INFLUX_IMAGE
